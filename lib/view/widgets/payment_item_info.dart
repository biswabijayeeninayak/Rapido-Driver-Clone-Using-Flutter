...
class PaymentItemInfo extends StatelessWidget {
  ...
  const PaymentItemInfo({Key? key, required this.title,  this.icon, required this.amount,  this.isSubTotal = false,  this.isFromTripDetails = false, this.paymentType}) : super(key: key);
    ...
    return Padding(
      ...
      child: Row(children: [
        ...
        Container(padding: const EdgeInsets.symmetric(horizontal: Dimensions.paddingSizeSmall, vertical: Dimensions.paddingSizeExtraSmall),
            ...
            child: Text(PriceConverter.convertPrice(context, amount), style: textSemiBold.copyWith(color: Theme.of(context).primaryColorDark)))
            :
        paymentType!=null?
            Text(paymentType!,style: textRegular.copyWith(color: Theme.of(context).primaryColor)):
        Text(PriceConverter.convertPrice(context, amount), style: textRegular.copyWith(color: Theme.of(context).hintColor)),
